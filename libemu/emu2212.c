...
SCC_writeReg (SCC * scc, e_uint32 adr, e_uint32 val)
{
  ...
  else if (0xF0 <= adr && adr <=0xF4) scc->ch_pan[adr&0xF] = val;
...
INLINE static void
write_compatible (SCC * scc, e_uint32 adr, e_uint32 val)
{
  if ((adr & 0xF800) != 0x9800)
    return;
...
INLINE static int
mapper_write (SCC * scc, e_uint32 adr, e_uint32 val)
{
...
EMU2212_API void
SCC_calc_stereo (SCC * scc, e_int16 buf[2]) {
